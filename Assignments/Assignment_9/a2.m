...
function [hid_input, hid_output, class_input, log_class_prob, class_prob] = forward_pass(model, data)
...
function ret = loss(model, data, wd_coefficient)
  ...
  wd_loss = (sum(model.input_to_hid(:).^2) + sum(model.hid_to_class(:).^2))/2*wd_coefficient; % weight decay loss. very straightforward: E = 1/2 * wd_coeffecient * parameters^2
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  error = class_prob - data.targets;
  backpropagation_error = model.hid_to_class' * error;
  gradient = data.inputs * ((1.0 - hid_output) .* hid_output .* backpropagation_error)' / N;
   
  ret.input_to_hid = gradient';
  ret.hid_to_class = (hid_output * error')' / N;
...
function ret = model_to_theta(model)
  w% This function takes a model (or gradient in model form), and turns it into one long vector. See also theta_to_model.
  ret = [model.input_to_hid(:); model.hid_to_class(:)];
end
...
function ret = theta_to_model(theta)
  ...
  ret.input_to_hid = reshape(theta(1: 256*n_hid), n_hid, 256);
  ret.hid_to_class = reshape(theta(256 * n_hid + 1 : size(theta,1)), 10, n_hid);
end
